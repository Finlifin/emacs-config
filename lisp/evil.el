(use-package evil
  :defer (if (eq major-mode 'vterm-mode) (evil-emacs-state)))
(evil-mode)
(define-key evil-normal-state-map (kbd "<SPC>t") 'vterm)
(define-key evil-normal-state-map (kbd "<SPC>s") 'evil-search-forward)
(define-key evil-normal-state-map (kbd "<SPC>f") 
'(lambda () 
(interative) 
(if (eq major-mode 'emacs-lisp-mode) (eglot-format-buffer) (eglot-format-buffer))))
